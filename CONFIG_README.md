...
| `videodb_users.email` | movies-prisma-lib & movies-ui | Curated database email used for OAuth session authorization. The matching row provides `ownerId` and the legacy `asp_username` value. | `libs/movies-prisma-lib/src/lib/get-user-by-email.ts` · `apps/movies-ui/src/app/services/actions/getAllowedSession.ts` |
...
| `COVER_IMAGE_PATH` | movies-ui | Filesystem location for cover image assets served via `/api/cover-image/[id]`. | `apps/movies-ui/src/app/api/cover-image/[id]/route.ts:7` · `apps/movies-ui/src/app/services/actions/upsertVideoData.ts:196` |
